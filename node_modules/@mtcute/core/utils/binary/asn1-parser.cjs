...
function parsePemContents(pem) {
  return utils.base64.decode(pem.replace(/^-----(?:BEGIN|END)(?: RSA)? PUBLIC KEY-----$|\n/gm, ""));
}
const ELOOPN = 102;
...
function parseAsn1(data) {
  function parseAsn1Inner(buf, depth, eager = false) {
    ...
    if (128 & asn12.length) {
      ...
      asn12.length = Number.parseInt(utils.hex.encode(buf.subarray(index, index + asn12.lengthSize)), 16);
...
exports.parsePemContents = parsePemContents;
