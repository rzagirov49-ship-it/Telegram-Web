...
class BaseTelegramClient {
  constructor(params) {
    ...
    this.mt.onNetworkChanged.add((connected) => {
      if (!connected) {
  ...
  platform;
  timers;
  onServerUpdate = new utils.Emitter();
  onRawUpdate = new utils.Emitter();
  onConnectionState = new utils.Emitter();
  appConfig = new appConfigManager.AppConfigManager(this);
  ...
  async importSession(session, force = false) {
    ...
    const data = stringSession.readStringSession(session);
    const testMode = data.primaryDcs.main.testMode;
    if (testMode && !this.params.testMode) {
      throw new Error(
        `This session string is not for the current backend. Session is ${testMode ? "test" : "prod"}, but the client is ${this.params.testMode ? "test" : "prod"}`
      );
    }
  ...
  async exportSession() {
    ...
    return stringSession.writeStringSession({
      version: 3,
  ...
  computeSrpParams(request, password$1) {
    return password.computeSrpParams(this.crypto, request, password$1);
  }
  computeNewPasswordHash(algo, password$1) {
    return password.computeNewPasswordHash(this.crypto, algo, password$1);
  }
  get stopSignal() {
