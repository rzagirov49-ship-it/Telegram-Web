...
const pool = require("../arrays/u8/pool.cjs");
...
function decode(base64, url = false) {
  ...
  const k = rem && rem - 1;
  const m = (n >> 2) * 3 + k;
  const encoded = pool.alloc(n + 3);
  utf8.encoder.encodeInto(`${base64}===`, encoded);
...
const TO_BASE64_OPTIONS = { alphabet: "base64", omitPadding: false };
const TO_BASE64_URL_OPTIONS = { alphabet: "base64url", omitPadding: true };
function encode(bytes, url = false) {
  ...
  let base64 = utf8.decoder.decode(new Uint8Array(encoded.buffer, encoded.byteOffset, n));
  if (url) {
...
exports.decodedLength = decodedLength;
