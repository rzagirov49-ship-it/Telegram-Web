...
async function reorderUsernames(client, peerId, order) {
  ...
  if (peerUtils.isInputPeerUser(peer)) {
    if (utils.isSelfPeer(client, peer)) {
      ...
      typeAssertions.assertTrue("account.reorderUsernames", r2);
      return;
    }
    const r = await client.call({
      _: "bots.reorderUsernames",
      bot: peerUtils.toInputUser(peer, peerId),
      order
    });
    typeAssertions.assertTrue("bots.reorderUsernames", r);
  } else if (peerUtils.isInputPeerChannel(peer)) {
    const r = await client.call({
      _: "channels.reorderUsernames",
      channel: peerUtils.toInputChannel(peer, peerId),
      order
    });
    typeAssertions.assertTrue("channels.reorderUsernames", r);
  }
}
