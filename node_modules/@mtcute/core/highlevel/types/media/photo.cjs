...
class Photo extends fileLocation.FileLocation {
  constructor(raw, media) {
    ...
    if (progressive) {
      ...
      if (max) {
        ...
        throw new errors.MtArgumentError("Photo does not have any sizes");
      }
    }
   ...
   */
  get thumbnails() {
    const res = this.raw.sizes.map((sz) => new thumbnail.Thumbnail(this.raw, sz));
    this.raw.videoSizes?.forEach((sz) => res.push(new thumbnail.Thumbnail(this.raw, sz)));
    return res;
  }
  /**
  ...
  get fileId() {
    if (!this._bestSize) {
      throw new errors.MtArgumentError("Cannot get File ID for this photo");
    }
...
const Photo$1 = /* @__PURE__ */ memoize.memoizeGetters(Photo, ["thumbnails", "fileId", "uniqueFileId"]);
const Photo$2 = /* @__PURE__ */ inspectable.makeInspectable(Photo$1, ["fileSize", "dcId", "width", "height"], ["inputMedia", "inputPhoto"]);
exports.Photo = Photo$2;
