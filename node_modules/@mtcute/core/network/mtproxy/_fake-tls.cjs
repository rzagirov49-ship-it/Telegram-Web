...
function _getDoubleX(x, mod) {
  ...
  denominator = utils.bigint.modInv(denominator, mod);
  numerator = numerator * denominator % mod;
  return numerator;
}
function _isQuadraticResidue(a) {
  const r = utils.bigint.modPowBinary(a, QUAD_RES_POW, QUAD_RES_MOD);
  return r === 1n;
}
...
class TlsHelloWriter {
  ...
  key() {
    for (; ; ) {
      ...
      key[31] &= 127;
      let x = utils.bigint.fromBytes(key);
      const y = _getY2(x, KEY_MOD);
      if (_isQuadraticResidue(y)) {
        for (let i = 0; i < 3; i++) {
          x = _getDoubleX(x, KEY_MOD);
        }
        const key2 = utils.bigint.toBytes(x, 32, true);
        this.string(key2);
        return;
      }
    }
...
class FakeTlsPacketCodec {
  ...
  async encode(packet, into) {
    const tmp = io.Bytes.alloc(packet.length);
    ...
    while (tmp.available > 0) {
      ...
      if (this._isFirstTls) {
        ...
        packet2 = utils.u8.concat2(this._tag, tmp.readSync(MAX_TLS_PACKET_LENGTH - this._tag.length));
      } else {
        packet2 = tmp.readSync(MAX_TLS_PACKET_LENGTH);
      }
      utils.typed.toDataView(header).setUint16(3, packet2.length);
...
exports.FakeTlsPacketCodec = FakeTlsPacketCodec;
exports.generateFakeTlsHeader = generateFakeTlsHeader;
